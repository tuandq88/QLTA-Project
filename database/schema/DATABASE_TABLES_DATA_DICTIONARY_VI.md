...
| Bảng | Công dụng | Khóa chính | Cột liên kết chính |
|---|---|---|---|
| `case_files` | Hồ sơ vụ án/vụ việc trung tâm. `court_id` là tòa án đang quản lý/xét xử hồ sơ hiện tại; với án phúc thẩm là tòa phúc thẩm/current court. `first_instance_court_id` lưu tòa án đã xét xử sơ thẩm của hồ sơ phúc thẩm. `case_group/case_group_id` dùng cho cấp xét xử `SO_THAM`/`PHUC_THAM`; không dùng `case_type`, `procedure_law` hoặc `current_stage` để chứa cấp xét xử. | `case_id` | `court_id -> courts.court_id`, `first_instance_court_id -> courts.court_id`, `created_by/updated_by -> users.user_id`, các cột danh mục `case_type_id`, `case_status_id`, `case_group_id`, `procedure_law_id`, `current_stage_id`, `resolution_status_id` |
