...
$ResultFile = Join-Path $ScriptDir "APPELLATE_XXPT_RESULT_EXCEL_IMPORT_CHECK_RESULT.md"
...
function Write-Result {
    ...
    $lines = @(
        ...
        "",
        '```powershell',
        ".\tests\database\run_appellate_xxpt_result_excel_import_check.ps1 -DatabaseName $DatabaseName",
        '```',
        "",
        "## Log chi tiet",
        ""
...
$steps = @(
    ...
    [pscustomobject]@{ Name = "seed 033 excel events decisions appellate results"; Path = Join-Path $RepoRoot "database\seed\033_excel_seed_case_events_and_resolutions.sql" },
    [pscustomobject]@{ Name = "test appellate XXPT result import"; Path = Join-Path $ScriptDir "appellate_xxpt_result_excel_import_test.sql" }
)
...
Write-Host "Hoan thanh kiem tra import Excel ket qua XXPT. Ket qua: $ResultFile"
